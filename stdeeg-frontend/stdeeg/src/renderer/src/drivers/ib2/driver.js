...
// bufer
const buffer = Buffer.allocUnsafe(2048); // 2KB
let bufSize = 0;
        ...
        if(bufSize >= 208) {
          ...
          for(let i = 0; i<bufSize; ++i) {
            if(validate(buffer, i)) {
              ...
              buffer.set(buffer.subarray(i+105, bufSize));
              bufSize -= 104;
              break;
            }
          }
...
const pause = (port) => { // bug
  port.write(new Uint8Array([ 0x55, 0xAA, 0x00, 0x55, 0xAA ])); // pause
  console.log('pause invoked');
};

const stop = async (port) => {
  await port.write(new Uint8Array([ 0xAA, 0xAA, 0x02, 0x0A, 0x64, 0x69, 0x73, 0x63, 0x6F, 0x6E, 0x6E, 0x65, 0x63, 0x74, 0xD5 ]));
  await port.close();
